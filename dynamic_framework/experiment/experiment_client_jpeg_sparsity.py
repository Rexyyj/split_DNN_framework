...
import math
import os
from split_framework.split_framework_dynamic_sparsity import SplitFramework
import tqdm
import numpy as np
...
from algorithm.single_manager_sparsity import Manager
################################### Varialbe init ###################################
...
def write_time_data(sf, manager,tech,bandwidth, mAP_drop,frame_id):
    ...
    quality, sparsity = manager.get_configuration()
...
def write_characteristic(sf, manager,tech,bandwidth,mAP_drop,frame_id):
    ...
    quality, target_sparsity = manager.get_configuration()
    thresh = sf.get_pruning_thresh()
...
def write_map( sf, manager, tech,bandwidth,mAP_drop,frame_id,feasibility,sensitivity,map_value):
    thresh = sf.get_pruning_thresh()
    quality, target_sparsity = manager.get_configuration()
    if __COMPRESSION_TECHNIQUE__ =="sketchml":
        quality= str(quality[0])+"-"+str(quality[1])+"-"+str(quality[2])
        ...
        for i in range(1):
            ...
            for _, imgs, targets in tqdm.tqdm(dataloader, desc="testing"):
                ...
                try:
                    ...
                    cmp = 0
                manager.update_sample_points([sparsity,quality],cmp,sf.get_reconstruct_snr())
                
                detection = sf.split_framework_client(imgs,service_uri=service_uri)
                write_time_data(sf,manager,technique,available_bandwidth,mAP_drop,frame_index)
                ...
                ## Save data
                write_map(sf, manager,technique,available_bandwidth,mAP_drop,frame_index,fesiable,sensitivity,AP.mean())
