...
class SplitFramework():
    ...
    def encode_bool_tensor_to_byte(self, tensor):
        tensor = tensor.reshape(64*208*208)
    ...
    def regression_encode(self,tensor):
        ...
        zero_mask_bytes = self.encode_bool_tensor_to_byte(zero_mask.reshape(64*208*208))
        negative_mask_bytes = self.encode_bool_tensor_to_byte(negative_mask.reshape(64*208*208))

        return zero_mask_bytes, negative_mask_bytes, coffe, reconstructed_tensor

    def regression_decode(self, tensor_dict):
        zero_mask = self.decode_byte_to_bool_tensor(tensor_dict["zero_mask"], 64*208*208)
        zero_mask = zero_mask.reshape([64,43264])
        negative_mask = self.decode_byte_to_bool_tensor(tensor_dict["negative_mask"], 64*208*208)
