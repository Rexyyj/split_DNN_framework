...
class SplitFramework():
    ...
    def split_framework_encode(self,id, head_tensor):
        with torch.no_grad():
            ...
            # pruner
            # diff_tensor_normal = torch.nn.functional.normalize(head_tensor)
            # pruned_tensor = head_tensor*(abs(diff_tensor_normal) > self.pruning_threshold)

            normalize_base, scale,zero_point, encoded_data, reconstructed_tensor = self.jpeg_encode(head_tensor)
