...
import numpy as np
from pytorchyolo.utils.utils import load_classes, rescale_boxes, non_max_suppression, print_environment_info,xywh2xyxy_np,get_batch_statistics,ap_per_class
import pandas as pd
import time
import torch
...
class_names = ["ball","goalkeeper","referee","soccer-player"]

test_case = "football_tensor_jpeg_v2"
...
def load_ground_truth():
    ...
    for test_file in test_files:
        ...
        for gt in gts:
            ...
            global_labels.append(temp[0])
            box = xywh2xyxy_np(np.array(temp[1:]))*416
            labels.append([0,temp[0],box[0],box[1],box[2],box[3]])
        frame_labels.append(tensor(labels,dtype=torch.float32))
    return frame_labels
...
if __name__ == "__main__":
    ...
    for index in range(len(test_frames)):
        ...
        if len(detection[0])!= 0:
            ...
            sample_metrics += get_batch_statistics(detection, frame_labels[index], iou_threshold=0.5)
            # print("------------------------------------")
            # print(pred)
            # print("***********")
    ...
    metrics_output = ap_per_class(
        true_positives, pred_scores, pred_labels, global_labels)
