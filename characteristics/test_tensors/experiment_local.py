...
################################### Varialbe init ###################################
testdata_path = "../../St_Marc_dataset/data/coco_val_small.txt"
...
if __name__ == "__main__":
    # Load Model
    model = models_split_tiny.load_model("../../pytorchyolo/config/yolov3.cfg","../../pytorchyolo/weights/yolov3.weights")
    split_layer = 90
    model.set_split_layer(split_layer-1) # layer <7
    model = model.eval()
    ...
    for j in range(1):
        for i in range(1):
            ...
            for _, imgs, targets in tqdm.tqdm(dataloader, desc="testing"):
                frame_index+=1
                print("Working on tenosr: ", frame_index)
                # Real measurements
                # Extract labels
                ...
                with torch.no_grad():
                    ...
                    print(head_tensor.shape)
                    torch.save(head_tensor.cpu(),"./coco_tensor_yolov3/layer_"+str(split_layer)+"/"+str(frame_index)+".pt")
